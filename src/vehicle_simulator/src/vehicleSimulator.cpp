...
#include <stdlib.h>
#include <ros/ros.h>
#include <Eigen/Dense> //extra line
...
//Eigen::Matrix3f rotationMatrix;  // Global rotation matrix, Radha Rani
#include <limits>
using namespace std;
Eigen::Vector3f prpoint; //Radha Rani
...
//float vehicleRecY;
//float vehicleRecZ;
//Eigen::Vector3f prpoint;  // Declare globally without initializin
//Jai shree Ram

// Define waypoints as a vector of pairs (each pair is a waypoint with x and y coordinates)
...
// Initialize the current waypoint index to 0
...
{
  vehicleSpeed = 0.5*speedIn->twist.linear.x;
  vehicleYawRate = 0.5*speedIn->twist.angular.z;
}
...
void goalHandlerR(const geometry_msgs::PointStamped::ConstPtr& goal) //Radha Rani3
{
  ...
  float cosTerrainRecPitch = cos(terrainRecPitch);
  // Apply rotation
  prpoint = Eigen::Vector3f(goal->point.x, goal->point.y, goal->point.z);
  
  prpoint = rotationMatrix * prpoint;

  // Apply translation
  prpoint.x() += vehicleRecX;
  prpoint.y() += vehicleRecY;
  prpoint.z() += vehicleRecZ;
  std::cout<<"Goal Loop:"<<prpoint.x()<<","<<prpoint.y()<<std::endl;
...
int main(int argc, char** argv)
{
  ...
  ros::Publisher pubWayPoint = nh.advertise<geometry_msgs::PointStamped> ("/goal_point", 5); //Radha Rani @FAR

  ros::Subscriber subGoal = nh.subscribe<geometry_msgs::PointStamped> ("/way_point", 5, goalHandlerR); //Radha Rani1
  ...
  while (status)
  {
    ...
    //cameraState.pose.position.z = 0.2475 + cameraOffsetZ;
    ...
    robotState.pose.position.z = 0.2475;
    ...
    //lidarState.pose.position.z = 0.5575;
    ...
    if (manFound == true)
    {
      ...
      if (std::abs(prpoint.x() - vehicleX) < 0.1f && std::abs(prpoint.y() - vehicleY) < 0.1f) {
        point_msg.point.x = std::numeric_limits<float>::quiet_NaN();
        point_msg.point.y = std::numeric_limits<float>::quiet_NaN();
      }else{
        point_msg.point.x = prpoint.x();
        point_msg.point.y = prpoint.y();
      }
      point_msg.point.z = 0;
      point_msg.header.stamp = ros::Time::now();
      point_msg.header.frame_id = "map";  // replace with the correct frame
      pubWayPoint.publish(point_msg);
      std::cout<<"Goal publish:"<<prpoint.x()<<","<<prpoint.y()<<std::endl;
    }else
    {
      // Assuming waypoints and current_waypoint_index are defined as above
      float px = waypoints[current_waypoint_index].first;  // x-coordinate
      float py = waypoints[current_waypoint_index].second; // y-coordinate
      // Set the point coordinates
      ...
      point_msg.header.frame_id = "map";  // replace with the correct frame
      pubWayPoint.publish(point_msg);
      if (std::abs(px - vehicleX) < 0.3f && std::abs(py - vehicleY) < 0.3f) {
        current_waypoint_index += 1;
        if (current_waypoint_index >= static_cast<int>(waypoints.size())) {
          current_waypoint_index = -1;
        }
      }
      std::cout<<"Predeined Location Visiting for Search:"<<px<<","<<py<<std::endl;
      manFound = false;
    }
    */
    status = ros::ok();
