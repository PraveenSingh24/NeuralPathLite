...
#include "std_msgs/Bool.h" // Radha Rani
...
double goalXR = 0;
double goalYR = 0;
bool aligningToGoal = false;
bool global_stop_signal = false; // Radha Rani
...
// Callback function to handle incoming Bool messages
void stopCallback(const std_msgs::Bool::ConstPtr& msg)
{
    // Assign the message data to the global variable
    global_stop_signal = msg->data;

    if (global_stop_signal)
    {
        ROS_INFO("Stop signal received: True (Stop the robot)");
        // Add logic to stop the robot here
    }
    else
    {
        ROS_INFO("Stop signal received: False (Robot can continue)");
        // Add logic to continue robot movement
    }
}
...
int main(int argc, char** argv)
{
  ...
  ros::Subscriber sub = nh.subscribe("/vehicle/stop", 10, stopCallback); // Radha Rani
  ...
  // Initialize goalXR/goalYR to match goal so the goal-alignment rotation
  // block doesn't trigger spuriously at startup
  goalXR = goalX;
  goalYR = goalY;
  ...
  while (status) {
    ...
    if (pathInit) {
      ...
      */
      ...
      //Radha - Align-first logic (distance-based new goal detection)
      ...
      // Detect new goal by checking if it moved significantly (>0.5m)
      float goalShift = sqrt((goalX-goalXR)*(goalX-goalXR) + (goalY-goalYR)*(goalY-goalYR));
      float goalDis = sqrt((goalX-vehicleX)*(goalX-vehicleX) + (goalY-vehicleY)*(goalY-vehicleY));
      if (goalShift > 0.5 && goalDis > 0.5) {
        aligningToGoal = true;
        goalXR = goalX;
        goalYR = goalY;
      }
      // Persist rotation until aligned (yaw error < 0.15 rad ~8.6 deg)
      if (aligningToGoal && fabs(yawDiff) > 0.15) {
