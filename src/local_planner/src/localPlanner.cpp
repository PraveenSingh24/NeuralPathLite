...
pcl::PointCloud<pcl::PointXYZI>::Ptr
    freePaths(new pcl::PointCloud<pcl::PointXYZI>());
#endif
  ...
  for (int i = 0; i < boundarySize; i++) {
    ...
    if (point1.z == point2.z) {
      ...
      for (int pointID = 0; pointID < pointNum; pointID++) {
        point.x = float(pointID) / float(pointNum) * point1.x +
                  (1.0 - float(pointID) / float(pointNum)) * point2.x;
        point.y = float(pointID) / float(pointNum) * point1.y +
                  (1.0 - float(pointID) / float(pointNum)) * point2.y;
        point.z = 0;
...
int main(int argc, char **argv) {
  ...
  // ros::Subscriber subJoystick = nh.subscribe<sensor_msgs::Joy> ("/joy", 5,
  // joystickHandler);

  ros::Subscriber subGoal =
      nh.subscribe<geometry_msgs::PointStamped>("/wpts", 5, goalHandler);
    ...
    if (newLaserCloud || newTerrainCloud) {
      ...
      *///end
      ...
      if (autonomyMode) {
        ...
        // float relativeGoalX = ((10 - vehicleX) * cosVehicleYaw + (10 -
        // vehicleY) * sinVehicleYaw); float relativeGoalY = (-(10 - vehicleX) *
        // sinVehicleYaw + (10 - vehicleY) * cosVehicleYaw);
        ...
        */
        if (!twoWayDrive) {
          if (joyDir > 40.0)
            ...
            joyDir = -40.0; // we will examine it later.
        }
      }
      ...
      float h_fov_start = -40.0; // Half of 80 degrees to the left (-40 degrees)
      float h_fov_end = 40.0; // Half of 80 degrees to the right (+40 degrees)
      while (pathScale >= minPathScale && pathRange >= minPathRange) {
        for (int i = 0; i < 9 * pathNum; i++) {
          clearPathList[i] = 0;
          pathPenaltyList[i] = 0;
        }
        for (int i = 0; i < 9 * groupNum; i++) {
          clearPathPerGroupScore[i] = 0;
        }

        // float minObsAngCW = -180.0;
        // float minObsAngCCW = 180.0;
        float minObsAngCW = h_fov_start;
        float minObsAngCCW = h_fov_end;
        ...
        for (int i = 0; i < plannerCloudCropSize; i++) {
          ...
          // float h = plannerCloudCrop->points[i].z / pathScale;
          ...
          if (dis < pathRange / pathScale &&
              (dis <= (relativeGoalDis + goalClearRange) / pathScale ||
               !pathCropByGoal) &&
              checkObstacle) {
            for (int rotDir = 0; rotDir < 9; rotDir++) {
              float rotAng = (10.0 * rotDir - 40.0) * PI / 180;
              /*
              ...
              && fabs(joyDir) > 90.0 && dirToVehicle)) { continue;
              }
              */
              // Only evaluate paths within the h_fov of the camera
              float angDeg = rotAng * 180.0 / PI;
              if (angDeg < h_fov_start || angDeg > h_fov_end) {
                continue; // Skip angles outside the camera's field of view
              }

              float angDiff = fabs(joyDir - (10.0 * rotDir - 40.0));
              if (angDiff > 180.0) {
                angDiff = 360.0 - angDiff;
              }

              if ((angDiff > dirThre && !dirToVehicle) ||
                  (fabs(10.0 * rotDir - 40.0) > dirThre &&
        ...
        for (int i = 0; i < 9 * pathNum; i++) {
          ...
          if (clearPathList[i] < pointPerPathThre) {
            ...
            float rotDirW;
            if (rotDir < 5)
              rotDirW = fabs(fabs(rotDir - 0) + 1);
            else
              rotDirW = fabs(fabs(rotDir - 8) + 1);
            ...
            if (score > 0) {
              clearPathPerGroupScore[groupNum * rotDir +
                                     pathList[i % pathNum]] += score;
              // printf("\nNumber of Path Points: %u\n", pathList[i % pathNum]);
            }
          }
        }
        float maxScore = 0;
        ...
        if (selectedGroupID >= 0) {
          ...
          // Output the number of path points
          // printf("\nNumber of Path Points: %u\n", groupNum);
