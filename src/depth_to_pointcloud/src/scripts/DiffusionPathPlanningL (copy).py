...
class Online2DMapper:
    ...
    def is_path_blocked(self, p1, p2):
        """Check if the line segment p1->p2 crosses any obstacle in the current map."""
        u1, v1 = self.world_to_pixel(p1[0], p1[1])
        u2, v2 = self.world_to_pixel(p2[0], p2[1])
        num_samples = max(int(np.hypot(u2 - u1, v2 - v1)), 2)
        check_r = 2  # pixel radius

        for t in np.linspace(0, 1, num_samples):
            c = int(u1 + t * (u2 - u1))
            r = int(v1 + t * (v2 - v1))
            if c < 0 or c >= self.grid_size_pixels or r < 0 or r >= self.grid_size_pixels:
                continue
            r_min, r_max = max(0, r - check_r), min(self.grid_size_pixels, r + check_r + 1)
            c_min, c_max = max(0, c - check_r), min(self.grid_size_pixels, c + check_r + 1)
            if np.any(self.current_map_img[r_min:r_max, c_min:c_max] == 0):
                return True
        return False
    ...
    def timer_build_map(self, event):
        with self.map_lock:
            ...
            # Always update the map (collision is checked in publisher loop)
