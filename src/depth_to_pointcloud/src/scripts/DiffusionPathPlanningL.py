...
class Online2DMapper:
    def __init__(self):
        ...
        self.safety_margin = float(rospy.get_param("~safety_margin", 0.0))
        ...
        self.new_points_flag = False    # Set when map needs rebuild
        ...
        self.blocked_count = 0            # Consecutive blocked checks (debounce)
        ...
        rospy.Timer(rospy.Duration(1.0), self.timer_build_map)  # 1Hz for fresh map
    ...
    def is_path_blocked(self, p1, p2, map_img=None):
        """Check path against inflated map, then verify with raw point density."""
        if map_img is None:
            map_img = self.current_map_img
        u1, v1 = self.world_to_pixel(p1[0], p1[1])
        u2, v2 = self.world_to_pixel(p2[0], p2[1])
        n = max(int(np.hypot(u2 - u1, v2 - v1)), 2)
        ts = np.linspace(0, 1, n)
        cols = (u1 + ts * (u2 - u1)).astype(int)
        rows = (v1 + ts * (v2 - v1)).astype(int)
        valid = (cols >= 0) & (cols < self.grid_size_pixels) & \
                (rows >= 0) & (rows < self.grid_size_pixels)
        cols = cols[valid]
        rows = rows[valid]
        if len(cols) == 0:
            return False

        # Quick check: any obstacle pixels along the line?
        hit_mask = map_img[rows, cols] == 0
        if not np.any(hit_mask):
            return False

        # Obstacle pixels found — verify with raw point density
        # Convert hit pixels back to world coordinates
        hit_cols = cols[hit_mask]
        hit_rows = rows[hit_mask]
        hit_wx = (hit_cols - self.center_pixel) * self.grid_resolution
        hit_wy = (hit_rows - self.center_pixel) * self.grid_resolution

        # Check if real dense points exist near hit locations (not just noise)
        with self.map_lock:
            if self.kdtree is None:
                return False
            kdtree = self.kdtree

        # Sample a few hit points (max 5 to keep it fast)
        check_pts = np.column_stack([hit_wx, hit_wy])
        if len(check_pts) > 5:
            indices = np.linspace(0, len(check_pts) - 1, 5, dtype=int)
            check_pts = check_pts[indices]

        for pt in check_pts:
            neighbors = kdtree.query_ball_point(pt, r=0.5)
            if len(neighbors) >= 11:  # Dense cluster = real obstacle (1-10 = noise)
                return True

        # Only sparse noise points nearby — not a real obstacle
        return False

    def extract_corner_waypoints(self, path_points, angle_thresh_deg=150, min_dist=0.5):
    ...
    def cloud_callback(self, msg):
        ...
        with self.map_lock:
            ...
            if self.kdtree is None:
                ...
                dists, idxs = self.kdtree.query(pts, distance_upper_bound=self.dup_threshold)
                for i, (dist, idx) in enumerate(zip(dists, idxs)):
                    if np.isinf(dist):
                        self.global_points.append(pts[i].tolist())
                        added += 1
                    else:
                        self.global_points[idx] = pts[i].tolist()
                        replaced += 1
            ...
            if len(self.global_points) > self.max_points:
                ...
                self.new_points_flag = True
            elif added > 0:
                self.kdtree = cKDTree(np.array(self.global_points))
                self.new_points_flag = True

            # Replacements also need map rebuild (stale points corrected)
            if not self.new_points_flag and replaced > 0:
                self.new_points_flag = True
    ...
    def planner_loop(self):
        while not rospy.is_shutdown():
            ...
            try:
                # 1. Prepare map image (snapshot for both planning and collision check)
                plan_map = self.current_map_img.copy()

                map_color = cv2.cvtColor(plan_map, cv2.COLOR_GRAY2BGR)
                ...
                # 5. Find starting waypoint index (use current pose, not snap_start)
                robot = np.array(self.current_pose) if self.current_pose else np.array(snap_start)
                start_idx = 0
                min_d = float('inf')
                ...
                # 6. Accept path (server already planned around obstacles on same map)
                # Runtime collision is handled by publisher loop continuously
                self.blocked_count = 0  # Reset for fresh navigation
                ...
                wp = sparse_wps[start_idx]
                rospy.loginfo(f"[Nav] -> NAVIGATING. Waypoint {start_idx}/{len(sparse_wps)}: ({wp[0]:.2f}, {wp[1]:.2f})")
    ...
    # ================================================================
    #  WAYPOINT PUBLISHER (2 Hz, only during NAVIGATING)
    # ================================================================
        ...
        while not rospy.is_shutdown():
            if self.state == NavState.NAVIGATING and self.current_pose is not None:
                ...
                if wx is not None:
                    ...
                    if not blocked:
                        # Path clear -> publish waypoint, reset counter
                        ...
                        # Path blocked -> DON'T publish, wait
                        self.blocked_count += 1
                        if self.blocked_count <= 5:
                            rospy.logwarn(f"[Nav] Waypoint blocked ({self.blocked_count}/5), waiting...")
                        else:
                            rospy.logwarn("🚨 Persistent collision -> PLANNING")
                            self.blocked_count = 0
                            self.state = NavState.PLANNING
                            self.plan_event.set()
            r.sleep()
