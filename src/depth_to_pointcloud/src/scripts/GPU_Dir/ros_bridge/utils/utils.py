...
med_frqs = [0.382900, 0.452448, 0.637584, 0.377464, 0.585595,
...
label_coloursp = [(0, 0, 0),(255, 255, 255)]
"""""
...
def save_ckpt(ckpt_dir, model, optimizer, global_step, epoch, local_count, num_train):
    ...
    ckpt_model_filename = "radhaGovind.pth".format(epoch_float)
...
def load_ckpt(model, optimizer, model_file, device):
    if os.path.isfile(model_file):
        ...
        if device.type == 'cuda':
            ...
            print("ok Hare Krishna")
        else:
            checkpoint = torch.load(model_file, map_location=lambda storage, loc: storage)

        # Fix for "module." prefix issue
        state_dict = checkpoint['state_dict']
        new_state_dict = {key.replace("module.", ""): value for key, value in state_dict.items()}  # Remove "module."

        model.load_state_dict(new_state_dict)  # ✅ Use the modified state_dict
        if optimizer:
        ...
        os._exit(0)
"""""
def load_ckpt(model, optimizer, checkpoint_path, device):
    ...
    model.load_state_dict(new_state_dict)  # Load modified state_dict
